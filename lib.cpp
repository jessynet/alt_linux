    ...
    for(int i = 0; i < s1.size(); i++)
    {
        ...
        for(const auto& [name1, version1] : name_vers_b1)
        {
            if(name_vers_b2.count(name1)) //Если во второй ветке есть такой пакет (есть такой ключ)
            {
                ...
                //cout << strverscmp("p70", "p69"); 
                //cout << version1.data() << name_vers_b2[name1].data();
                //cout << strverscmp(version1.data(), name_vers_b2[name1].data());
...
void compare_branches(const char* path_temp_1, const char* path_temp_2)
{
    ...
    //for(auto p : json_packages_b1)
    //{
    //    for(auto i : p) cout << i << " ";
    //    cout << endl;
    //}
    
    //for(int i = 0; i < arch_b1.size(); i++)
    //{
        //cout << arch_packages_b1[i][0] << " ";
    //    for(auto p : arch_packages_b1[i]) cout << p << " ";
    //}

    //cout << endl;
    
    //for(auto s : arch_b1) cout << s << " ";

    //cout << endl;

    //for(auto s : arch_b2) cout << s << " ";

    //cout << endl;

    //cout << count_pack_br1 << " " << count_pack_br2 << endl;

    //cout << info_branch_1["packages"]; 
...
void get_branch_info(vector<string> cmd, int fd)
{
    ...
    switch(pid){
        ...
        case 0: //дочерний процесс
        {
            ...
            char* args[cmd.size() + 1]; //массив char* строк
